...
from mmgpt.utils.evaluation_tools.vqa_annls_evaluator import VQAEval
...
def eval():
    ...
    data = json.load(open(data_args.eval_file_path, encoding='utf-8'))
    preds, gts = {}, {}
    ...
    for idx, line in enumerate(tqdm(data)):
        input_ids, image_tensor, stopping_criteria = prepare_input(
            ...
            line['question'],
            data_args.eval_image_path + line['image']
        )
        ...
        print(f"{idx}: Question: {line['question']} Ground Truth: {line['answer']} Answer: {outputs}")
        preds[line['question_id']] = outputs
        gts[line['question_id']] = line['answer']

    vqaEval = VQAEval(gts, preds, "DocVQA", n=2)  # n is precision of accuracy (number of places after decimal), default is 2
