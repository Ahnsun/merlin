...
class Resampler(nn.Module):
    ...
    def __init__(
        ...
        # set manually auto grad for hacking fsdp
        self.pos_embed = nn.Parameter(
            torch.from_numpy(get_2d_sincos_pos_embed(embed_dim, grid_size)).float()
        )
    ...
    def forward(self, x, attn_mask=None):
        ...
        out = self.attn(
            self._repeat(q, N) + self.pos_embed.detach().unsqueeze(1),
            x + pos_embed.detach().unsqueeze(1),
            x,
...
class QWenNoSamplerVisionTower(nn.Module):
    def __init__(self, args):
        ...
        self.vision_tower_name_or_path = args.vision_tower
        self.freeze_vision_tower = args.freeze_vision_tower

        self.image_processor = CLIPImageProcessor.from_pretrained(self.vision_tower_name_or_path)
        self.vision_tower = VisionTransformer(**self.visual_config)

        self.load_weights(self.vision_tower_name_or_path, args.model_name_or_path)
    ...
    def load_weights(self, vit_path, model_path):
        ...
        if os.path.exists(os.path.join(vit_path, 'pytorch_model.bin')):
            vision_tower_weight = {}
            prefix = 'transformer.visual.'

            ckpt = torch.load(os.path.join(vit_path, 'pytorch_model.bin'), map_location='cpu')
            for k in ckpt.keys():
                if prefix in k:
                    vision_tower_weight[k[len(prefix):]] = ckpt[k]

            missings = self.vision_tower.load_state_dict(vision_tower_weight, strict=False)
            logger.info(missings)
            logger.info(f"vision_tower weights (from {os.path.join(vit_path, 'pytorch_model.bin')}) are loaded!")
    ...
    def forward(self, images):
        with torch.set_grad_enabled(not self.freeze_vision_tower):
            image_features = []
            for image in images:
                image_feature = self.vision_tower(image.to(device=self.device, dtype=self.dtype)).to(image.dtype)
    ...
    @property
    def dtype(self):
        return self.vision_tower.transformer.get_cast_dtype()

    @property
    def device(self):
        return self.vision_tower.transformer.get_cast_device()
    ...
    def hidden_size(self):
        return self.visual_config['output_dim']

    @property
    def num_patches(self):
        # return (self.visual_config['image_size'] // self.visual_config['patch_size']) ** 2
        return 256
