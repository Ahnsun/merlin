...
###########
    ...
    def evaluate(self, quesIds=None):
        ...
        def eval_point_comp(gts, predictions, ids):
            '''
            for eval PointQA
            '''
            import re
            ...
            simple_gt, simple_pred= [],[]
            for i, qid in enumerate(ids):   
                gt_answer, pred = gts[qid], self.answer_processor(predictions[qid])
                pred = re.findall(r"\d+",pred) # extract number
                if not pred:
                    pred = [1]
                else:
                ...
                if gt_answer in pred:
                    pred = gt_answer
                else:
                    pred = pred[0]
                
                simple_gt.append(gt_answer)
        ...
        def eval_simp_comp(gts, predictions, ids):
            '''
            for eval TallyVQA
            '''
            import re
            ...
            for i, qid in enumerate(ids):   
                ...
                gt_answer, pred = gts[qid]['answer'], self.answer_processor(predictions[qid])
                pred = re.findall(r"\d+",pred) # extract number
                if not pred:
                    pred = [0]
                else:
                ...
                if gt_answer in pred:
                    ...
                    pred = pred[0]
                if gts[qid]['data_source'] == 'amt':
                    complex_gt.append(gt_answer)
        ...
        elif self.datatype in ANLS_metric_tasks:
            import editdistance
            score = 0
        ...
        elif self.datatype in EM_metric_tasks:
            quesIds = list(gts.keys())
            Sacc, Srmse, Cacc, Crmse = eval_simp_comp(self.vqa, self.vqaRes, quesIds)
            return Sacc, Srmse, Cacc, Crmse
        
        elif self.datatype in PointQA_metric_tasks:
            quesIds = list(gts.keys())
            Sacc, Srmse = eval_point_comp(self.vqa, self.vqaRes, quesIds)
            return Sacc, Srmse
        # -------------- end eval -------------- 
